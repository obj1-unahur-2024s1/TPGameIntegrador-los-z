import wollok.game.*
import level.*
import tile.*
import level12.* 



const level11 = new Level (
	solidTiles = [
	 //Horizontal walls (> 2 length)
        new SolidWall(
            start = new Coordinates(x = 0, y = 14),
            end = new Coordinates(x = 5, y = 14),
            orientation = "x"
        ),
        new SolidWall(
            start = new Coordinates(x = 9, y = 14),
            end = new Coordinates(x = 12, y = 14),
            orientation = "x"
        ),
        new SolidWall(
            start = new Coordinates(x = 12, y = 13),
            end = new Coordinates(x = 14, y = 13),
            orientation = "x"
        ),
        new SolidWall(
            start = new Coordinates(x = 5, y = 12),
            end = new Coordinates(x = 7, y = 12),
            orientation = "x"
        ),
        new SolidWall(
            start = new Coordinates(x = 7, y = 11),
            end = new Coordinates(x = 9, y = 11),
            orientation = "x"
        ),
        new SolidWall(
            start = new Coordinates(x = 15, y = 8),
            end = new Coordinates(x = 17, y = 8),
            orientation = "x"
        ),
        new SolidWall(
            start = new Coordinates(x = 8, y = 6),
            end = new Coordinates(x = 10, y = 6),
            orientation = "x"
        ),
        new SolidWall(
            start = new Coordinates(x = 15, y = 5),
            end = new Coordinates(x = 17, y = 5),
            orientation = "x"
        ),
        new SolidWall(
            start = new Coordinates(x = 5, y = 4),
            end = new Coordinates(x = 8, y = 4),
            orientation = "x"
        ),
        new SolidWall(
            start = new Coordinates(x = 3, y = 1),
            end = new Coordinates(x = 6, y = 1),
            orientation = "x"
        ),
        new SolidWall(
            start = new Coordinates(x = 1, y = 0),
            end = new Coordinates(x = 3, y = 0),
            orientation = "x"
        ),
        new SolidWall(
            start = new Coordinates(x = 6, y = 0),
            end = new Coordinates(x = 14, y = 0),
            orientation = "x"
        ),

        //vertical
        new SolidWall(
            start = new Coordinates(x = 0, y = 11),
            end = new Coordinates(x = 0, y = 13),
            orientation = "y"
        ),
        new SolidWall(
            start = new Coordinates(x = 0, y = 4),
            end = new Coordinates(x = 0, y = 8),
            orientation = "y"
        ),
        new SolidWall(
            start = new Coordinates(x = 1, y = 0),
            end = new Coordinates(x = 1, y = 4),
            orientation = "y"
        ),
        new SolidWall(
            start = new Coordinates(x = 1, y = 8),
            end = new Coordinates(x = 1, y = 11),
            orientation = "y"
        ),
        new SolidWall(
            start = new Coordinates(x = 3, y = 3),
            end = new Coordinates(x = 3, y = 5),
            orientation = "y"
        ),
        new SolidWall(
            start = new Coordinates(x = 4, y = 3),
            end = new Coordinates(x = 4, y = 9),
            orientation = "y"
        ),
        new SolidWall(
            start = new Coordinates(x = 10, y = 1),
            end = new Coordinates(x = 10, y = 5),
            orientation = "y"
        ),
        
        
      //single
	    new SolidTile(position = game.at(3, 9)),
        new SolidTile(position = game.at(5, 3)),
        new SolidTile(position = game.at(5, 13)),
        new SolidTile(position = game.at(6, 2)),
        new SolidTile(position = game.at(6, 3)),
        new SolidTile(position = game.at(6, 6)),
        new SolidTile(position = game.at(7, 9)),
        new SolidTile(position = game.at(9, 9)),
        new SolidTile(position = game.at(9, 10)),
        new SolidTile(position = game.at(9, 12)),
        new SolidTile(position = game.at(9, 13)),
        new SolidTile(position = game.at(10, 9)),
        new SolidTile(position = game.at(11, 1)),
        new SolidTile(position = game.at(12, 6)),
        new SolidTile(position = game.at(14, 1)),
        new SolidTile(position = game.at(14, 2)),
        new SolidTile(position = game.at(14, 11)),
        new SolidTile(position = game.at(14, 12)),
        new SolidTile(position = game.at(15, 2)),
        new SolidTile(position = game.at(15, 3)),
        new SolidTile(position = game.at(15, 10)),
        new SolidTile(position = game.at(15, 11)),
        new SolidTile(position = game.at(16, 3)),
        new SolidTile(position = game.at(16, 4)),
        new SolidTile(position = game.at(16, 9)),
        new SolidTile(position = game.at(16, 10)),
        new SolidTile(position = game.at(17, 6)),
        new SolidTile(position = game.at(17, 7))
	     
	    
    ],
     
    iceTiles = [
	    //Horizontal
        new IceWall(
            start = new Coordinates(x = 9, y = 8),
            end = new Coordinates(x = 14, y = 8),
            orientation = "x"
        ),
        new IceWall(
            start = new Coordinates(x = 8, y = 7),
            end = new Coordinates(x = 16, y = 7),
            orientation = "x"
        ),
        new IceWall(
            start = new Coordinates(x = 13, y = 6),
            end = new Coordinates(x = 16, y = 6),
            orientation = "x"
        ),
        new IceWall(
            start = new Coordinates(x = 12, y = 5),
            end = new Coordinates(x = 14, y = 5),
            orientation = "x"
        ),
        new IceWall(
            start = new Coordinates(x = 11, y = 4),
            end = new Coordinates(x = 13, y = 4),
            orientation = "x"
        ),
        new IceWall(
            start = new Coordinates(x = 11, y = 2),
            end = new Coordinates(x = 13, y = 2),
            orientation = "x"
        ),
        
        //Vertical
        new IceWall(
            start = new Coordinates(x = 1, y = 5),
            end = new Coordinates(x = 1, y = 7),
            orientation = "y"
        ),
        new IceWall(
            start = new Coordinates(x = 2, y = 2),
            end = new Coordinates(x = 2, y = 13),
            orientation = "y"
        ),
        new IceWall(
            start = new Coordinates(x = 3, y = 6),
            end = new Coordinates(x = 3, y = 8),
            orientation = "y"
        ),
        new IceWall(
            start = new Coordinates(x = 3, y = 10),
            end = new Coordinates(x = 3, y = 13),
            orientation = "y"
        ),
        new IceWall(
            start = new Coordinates(x = 4, y = 10),
            end = new Coordinates(x = 4, y = 13),
            orientation = "y"
        ),
        new IceWall(
            start = new Coordinates(x = 5, y = 5),
            end = new Coordinates(x = 5, y = 11),
            orientation = "y"
        ),
        new IceWall(
            start = new Coordinates(x = 6, y = 7),
            end = new Coordinates(x = 6, y = 9),
            orientation = "y"
        ),
        new IceWall(
            start = new Coordinates(x = 7, y = 1),
            end = new Coordinates(x = 7, y = 3),
            orientation = "y"
        ),
        new IceWall(
            start = new Coordinates(x = 8, y = 1),
            end = new Coordinates(x = 8, y = 3),
            orientation = "y"
        ),
        new IceWall(
            start = new Coordinates(x = 9, y = 1),
            end = new Coordinates(x = 9, y = 5),
            orientation = "y"
        ),
        new IceWall(
            start = new Coordinates(x = 10, y = 10),
            end = new Coordinates(x = 10, y = 13),
            orientation = "y"
        ),
        new IceWall(
            start = new Coordinates(x = 11, y = 9),
            end = new Coordinates(x = 11, y = 13),
            orientation = "y"
        ),
        new IceWall(
            start = new Coordinates(x = 12, y = 9),
            end = new Coordinates(x = 12, y = 12),
            orientation = "y"
        ),
        
        

	     
	    //single
        new IceTile(position = game.at(1, 12)),
        new IceTile(position = game.at(1, 13)),
        new IceTile(position = game.at(4, 2)),
        new IceTile(position = game.at(6, 5)),
        new IceTile(position = game.at(6, 11)),
        new IceTile(position = game.at(7, 6)),
        new IceTile(position = game.at(7, 8)),
        new IceTile(position = game.at(7, 10)),
        new IceTile(position = game.at(8, 5)),
        new IceTile(position = game.at(8, 9)),
        new IceTile(position = game.at(8, 10)),
        new IceTile(position = game.at(11, 3)),
        new IceTile(position = game.at(11, 6)),
        new IceTile(position = game.at(12, 1)),
        new IceTile(position = game.at(12, 3)),
        new IceTile(position = game.at(13, 1)),
        new IceTile(position = game.at(13, 9)),
        new IceTile(position = game.at(13, 11)),
        new IceTile(position = game.at(13, 12)),
        new IceTile(position = game.at(14, 3)),
        new IceTile(position = game.at(14, 10)),
        new IceTile(position = game.at(15, 4)),
        new IceTile(position = game.at(15, 9))
        
       
    ],
      
    doubleTiles = [
        new DoubleTile(position = game.at(6, 10)),
        new DoubleTile(position = game.at(7, 5)),
        new DoubleTile(position = game.at(7, 7)),
        new DoubleTile(position = game.at(8, 8)),
        new DoubleTile(position = game.at(11, 5)),
        new DoubleTile(position = game.at(13, 3)),
        new DoubleTile(position = game.at(13, 10)),
        new DoubleTile(position = game.at(14, 4)),
        new DoubleTile(position = game.at(14, 9))
        
    ],
	     
	lockTiles = [new LockTile(position = game.at(3, 2))],
    keyTile = new KeyTile(position = game.at(16, 7)),
    initialTile = new Tile(position = game.at(7, 3)),
    bonusTile = new BonusTile(position = game.at(6, 11)),
    goalTile = new GoalTile(position = game.at(5, 2)),
    moveableTile = new MoveableTile(initialPos = game.at(7, 6)),
    plateTile = new PlateTile(position = game.at(2, 1)),
    doesntRequirePlate = false,
    plateIsPressed = false,
    nextLevel = level12,

    //unused
    portalTileA = new PortalTile(position = game.at(-1, -1), teleportTo = game.at(-1,-1)),
    portalTileB = new PortalTile(position = game.at(-1, -1), teleportTo = game.at(-1,-1))
)
